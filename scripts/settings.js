...
function loadValues() {
	...
	browser.storage.sync.get(['homepageURL', 'newTabURL', 'toolbarHeight', 'defaultPosition', 'iconTheme', 'hideMethod', 'excludedUrls', 'buttonOrder', 'checkboxStates', 'toolbarTransparency', 'buttonsInToolbarDiv']).then((result) => {
		...
		toolbarContainer.style.height = result.toolbarHeight / window.visualViewport.scale + 'px'
		menuContainer.style.height = result.toolbarHeight / window.visualViewport.scale + 'px'
...
function showTab(tabId) {
	...
	generalTab.style.borderColor = tabId === 'generalTab' ? 'cornflowerblue' : '#2b2a33'
	buttonsTab.style.borderColor = tabId === 'buttonsTab' ? 'cornflowerblue' : '#2b2a33'
	excludeTab.style.borderColor = tabId === 'excludeTab' ? 'cornflowerblue' : '#2b2a33'
}
